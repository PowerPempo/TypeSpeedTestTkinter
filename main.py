...
class Windows(tk.Tk):
    ...
    def __init__(
        ...
        self.mainframe.grid(column=0, row=0, sticky="nwes")  # Updated: Added sticky="nwes"
        self.columnconfigure(0, weight=1)  # Updated: Added column weight configuration
        self.rowconfigure(0, weight=1)     # Updated: Added row weight configuration

        # Text for text to write
        ttk.Label(self.mainframe, text="Text to type:").grid(column=0, row=0, sticky="w")  # Updated: Added sticky="w"
        self.text_window = tk.Text(self.mainframe, height=5, width=35, wrap="word")
        self.text_window.configure(state="disabled")
        self.text_window.grid(column=0, row=1, sticky="nwes")  # Updated: Added sticky="nwes"
        ...
        # Text entry
        ttk.Label(self.mainframe, text="User input:").grid(column=0, row=2, sticky="w")  # Updated: Added sticky="w"
        self.user_input_window = ttk.Entry(self.mainframe, textvariable=self.user_input, width=35)
        self.user_input_window.grid(column=0, row=3, sticky="nwes")  # Updated: Added sticky="nwes"

        # Add padding to widgets and configure resizing
        for child in self.mainframe.winfo_children():
            child.grid_configure(padx=5, pady=5)  # Ensure padding for each widget

        # Updated: Configure row and column weights for mainframe to allow resizing
    ...
    def check_text(self, *args):
        ...
        for count, char in enumerate(self.user_input.get()):
            ...
            if count < len(self.text_to_type.get())-1:
                ...
                self.text_windosw.insert("end", char, "wrong")
